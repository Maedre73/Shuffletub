...
// ─── Supabase ──────────────────────────────────────────────────
const SUPABASE_URL = "https://pgkomzodnhjbmsaepegu.supabase.co";
const SUPABASE_KEY = "sb_publishable_T46AGdbOGl_Z9-_y8JEJrw_vkvve0Bo";
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

// ─── Users ────────────────────────────────────────────────────
const USERS = [
  { id: "pablo",  name: "Pablo",  emoji: "👦", color: "#7a9aaa", gradient: "linear-gradient(135deg,#8aaabb,#7a9aaa)" },
  { id: "kiara",  name: "Kiara",  emoji: "👧", color: "#c8845a", gradient: "linear-gradient(135deg,#d4895e,#c8705a)" },
];
const FEED_MAX = 50;

// ─── Supabase helpers ─────────────────────────────────────────
// Shared tables: upsert/fetch a single row with id
async function dbGet(table, id) {
  const { data, error } = await supabase.from(table).select("data").eq("id", id).single();
  if (error) return null;
  return data?.data ?? null;
}
async function dbSet(table, id, value) {
  await supabase.from(table).upsert({ id, data: value }, { onConflict: "id" });
}
// user_data table: key = e.g. "pablo_seenCh"
async function userGet(key) {
  const { data, error } = await supabase.from("user_data").select("data").eq("key", key).single();
  if (error) return null;
  return data?.data ?? null;
}
async function userSet(key, value) {
  await supabase.from("user_data").upsert({ key, data: value }, { onConflict: "key" });
}
// Fetch all rows from a shared table
...
function LoadingScreen() {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      ...
      <div style={{ color: C.mid, fontSize: 14, fontFamily: "'Lato',sans-serif" }}>Cargando datos…</div>
    </div>
  );
}
...
export default function App() {
  ...
  // ── Active user ──
  const [currentUserId, setCurrentUserId] = useState("pablo");
  ...
  useEffect(() => {
    async function loadAll() {
      setLoading(true);
      // shared
      const [chs, vids, pods, srcs, its, movs] = await Promise.all([
        dbGetAll("channels"),
        dbGetAll("saved_videos"),
        dbGetAll("podcasts"),
        dbGetAll("sources"),
        dbGetAll("items"),
        dbGetAll("movies"),
      ...
      // per-user for default user
      await loadUserData("pablo");
      setLoading(false);
    }
    loadAll();
  }, []);
  ...
  const loadUserData = async (userId) => {
    const [sm, sc, sf, sv2, si, sinbox, today, str, hist] = await Promise.all([
      userGet(`${userId}_seenMovies`),
      userGet(`${userId}_seenCh`),
      userGet(`${userId}_seenFeed`),
      userGet(`${userId}_seenVids`),
      userGet(`${userId}_seenItems`),
      userGet(`${userId}_seenInbox`),
      userGet(`${userId}_today`),
      userGet(`${userId}_streak`),
      userGet(`${userId}_history`),
    ]);
    setSeenMovies(sm || []);
    setSeenCh(sc || {});
    setSeenFeed(sf || []);
    ...
    setStreak(str || { count: 0, lastDate: null });
    setHistory(hist || []);
  };
  ...
  // ── Save helpers ──
  const saveShared = async (table, arr) => {
    // Upsert all items
    for (const item of arr) {
      await dbSet(table, item.id, item);
    }
  };

  // ── Persist shared content when it changes ──
  // (we save on every mutation instead of useEffect to avoid race conditions)

  const saveUserData = useCallback(async (userId, patch) => {
    const saves = Object.entries(patch).map(([k, v]) => userSet(`${userId}_${k}`, v));
    await Promise.all(saves);
  }, []);

  // ── Generate ──
  const handleGenerate = async (skipVideo = false, skipContent = false) => {
    ...
    const newToday = { date: todayStr(), video, content, videoDone: false, contentDone: false, videoNote: "", contentNote: "" };
    ...
    await saveUserData(currentUserId, {
      seenCh: sCh, seenFeed: sFeed, seenVids: sVids,
      seenItems: sItems, seenInbox: sInbox, seenMovies: sMovies,
      today: newToday,
    });
    setSaving(false);
  };

  const markDone = async (which) => {
    ...
    if ((which === "video" && updated.contentDone) || (which === "content" && updated.videoDone)) {
      ...
      newStreak = { count: newCount, lastDate: t };
      newHistory = [{ date: t, video: updated.video, content: updated.content, videoNote: updated.videoNote, contentNote: updated.contentNote }, ...history.slice(0, 59)];
      setStreak(newStreak);
  ...
  const saveNote = async (which, note) => {
    const updated = { ...todayData, [`${which}Note`]: note };
    setTodayData(updated);
    setModal(null);
    await saveUserData(currentUserId, { today: updated });
  };

  // ── Shared content mutations (with DB save) ──
  const addChannel = async (ch) => {
    const next = [...channels, ch];
    setChannels(next);
    await dbSet("channels", ch.id, ch);
  };
  const updateChannel = async (ch) => {
    const next = channels.map(x => x.id === ch.id ? ch : x);
    setChannels(next);
    await dbSet("channels", ch.id, ch);
  };
  const deleteChannel = async (id) => {
    setChannels(p => p.filter(c => c.id !== id));
    const newSeenCh = { ...seenCh }; delete newSeenCh[id];
    setSeenCh(newSeenCh);
    await dbDelete("channels", id);
    await saveUserData(currentUserId, { seenCh: newSeenCh });
  };

  const addSavedVideo = async (v) => {
    const next = [...savedVideos, v];
    setSavedVideos(next);
    await dbSet("saved_videos", v.id, v);
  };
  const updateSavedVideo = async (v) => {
    setSavedVideos(p => p.map(x => x.id === v.id ? v : x));
    await dbSet("saved_videos", v.id, v);
  };
  const deleteSavedVideo = async (id) => {
    setSavedVideos(p => p.filter(x => x.id !== id));
    const newSeen = seenVids.filter(i => i !== id);
    setSeenVids(newSeen);
    await dbDelete("saved_videos", id);
    await saveUserData(currentUserId, { seenVids: newSeen });
  };

  const addPodcast = async (p) => {
    setPodcasts(prev => [...prev, p]);
    await dbSet("podcasts", p.id, p);
  };
  const updatePodcast = async (p) => {
    setPodcasts(prev => prev.map(x => x.id === p.id ? p : x));
    await dbSet("podcasts", p.id, p);
  };
  const deletePodcast = async (id) => {
    setPodcasts(p => p.filter(x => x.id !== id));
    await dbDelete("podcasts", id);
  };

  const addSource = async (s) => {
    setSources(prev => [...prev, s]);
    await dbSet("sources", s.id, s);
  };
  ...
  const deleteSource = async (id) => {
    ...
    setItems(p => p.filter(it => it.sourceId !== id));
    const newInbox = { ...seenInbox }; delete newInbox[id];
    setSeenInbox(newInbox);
    await dbDelete("sources", id);
    // also delete items for this source
    const toDelete = items.filter(it => it.sourceId === id);
    for (const it of toDelete) await dbDelete("items", it.id);
    await saveUserData(currentUserId, { seenInbox: newInbox });
  };

  const addItem = async (it) => {
    setItems(prev => [...prev, it]);
    await dbSet("items", it.id, it);
  };
  ...
  const deleteItem = async (id) => {
    ...
    const newSeen = seenItems.filter(i => i !== id);
    setSeenItems(newSeen);
    await dbDelete("items", id);
    await saveUserData(currentUserId, { seenItems: newSeen });
  };

  const addMovie = async (m) => {
    setMovies(prev => [...prev, m]);
    await dbSet("movies", m.id, m);
  };
  ...
  const deleteMovie = async (id) => {
    setMovies(p => p.filter(x => x.id !== id));
    const newSeen = seenMovies.filter(i => i !== id);
    setSeenMovies(newSeen);
    await dbDelete("movies", id);
    await saveUserData(currentUserId, { seenMovies: newSeen });
  };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Lato',sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 16px 60px" }}>
      ...
      <div style={{ width: "100%", maxWidth: 480, paddingTop: 40, marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {saving && <span style={{ color: C.light, fontSize: 11 }}>💾 guardando…</span>}
            {streak.count > 0 && (
            ...
            return (
              <button key={u.id} onClick={() => switchUser(u.id)}
                style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
                  padding: "9px 12px", borderRadius: 12, fontFamily: "inherit", fontSize: 13, fontWeight: active ? 700 : 400,
                  background: active ? u.gradient : C.card,
                  border: active ? "none" : `1.5px solid ${C.border}`,
                  color: active ? "#fff" : C.mid,
                  boxShadow: active ? `0 4px 16px ${u.color}40` : "none",
                  transition: "all .2s" }}>
                <span style={{ fontSize: 16 }}>{u.emoji}</span>
                {u.name}
                {active && <span style={{ fontSize: 10, opacity: 0.85, marginLeft: 2 }}>✓</span>}
              </button>
            );
          })}
        </div>
      ...
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        {tab === "today" && (
          <div>
            {!isToday && (
              <button onClick={() => handleGenerate()} style={{ width: "100%", padding: "17px", background: currentUser.gradient, borderRadius: 14, color: "#fff", fontFamily: "'Playfair Display',serif", fontSize: 19, marginBottom: 22, boxShadow: `0 6px 28px ${currentUser.color}50` }}>
                {currentUser.emoji} Generar para {currentUser.name} hoy
              </button>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 24 }}>
              {channels.map(ch => (
                <div key={ch.id} style={{ display: "flex", alignItems: "center", ...card, padding: "11px 14px" }}>
                  ...
                  <RowAct onEdit={() => setModal({ type: "editCh", ch })} onDelete={() => deleteChannel(ch.id)} />
                </div>
              ))}
            </div>
              ...
              {savedVideos.map(v => {
                ...
                return (
                  <div key={v.id} style={{ display: "flex", alignItems: "center", ...card, padding: "11px 14px", opacity: done ? 0.5 : 1 }}>
                    ...
                    <RowAct onEdit={() => setModal({ type: "editVid", vid: v })} onDelete={() => deleteSavedVideo(v.id)} />
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 24 }}>
              {podcasts.map(p => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", ...card, padding: "11px 14px" }}>
                  ...
                  <RowAct onEdit={() => setModal({ type: "editPod", pod: p })} onDelete={() => deletePodcast(p.id)} />
                </div>
              ))}
            </div>
              ...
              {sources.map(s => {
                ...
                return (
                  <div key={s.id} style={{ display: "flex", alignItems: "center", ...card, padding: "11px 14px" }}>
                    ...
                    <RowAct onEdit={() => setModal({ type: "editSrc", src: s })} onDelete={() => deleteSource(s.id)} />
                  </div>
                );
              })}
            </div>
              ...
              {items.map(it => {
                ...
                return (
                  <div key={it.id} style={{ display: "flex", alignItems: "center", ...card, padding: "11px 14px", opacity: done ? 0.5 : 1 }}>
                    ...
                    <RowAct onEdit={() => setModal({ type: "editItem", item: it })} onDelete={() => deleteItem(it.id)} />
                  </div>
                );
              })}
            </div>
                ...
                {movies.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{ display: "flex", alignItems: "center", ...card, padding: "11px 14px", opacity: done ? 0.5 : 1 }}>
                      ...
                      <RowAct onEdit={() => setModal({ type: "editMov", mov: m })} onDelete={() => deleteMovie(m.id)} />
                    </div>
                  );
                })}
              </div>
        ...
        {tab === "stats" && (
          <div>
            <div style={{ ...lbl, marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
              <span>{currentUser.emoji}</span> Stats de {currentUser.name}
            </div>
            <div style={{ ...card, padding: "16px 18px", marginBottom: 24, borderLeft: `4px solid ${currentUser.color}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      {/* Modals */}
      {modal?.type === "newCh"    && <ChannelModal    onSave={ch  => { addChannel(ch);    setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "editCh"   && <ChannelModal ch={modal.ch}  onSave={ch  => { updateChannel(ch);    setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "newVid"   && <SavedVideoModal onSave={v   => { addSavedVideo(v);   setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "editVid"  && <SavedVideoModal vid={modal.vid} onSave={v => { updateSavedVideo(v); setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "newPod"   && <PodcastModal    onSave={p   => { addPodcast(p);      setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "editPod"  && <PodcastModal pod={modal.pod} onSave={p => { updatePodcast(p);    setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "newSrc"   && <SourceModal     onSave={s   => { addSource(s);       setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "editSrc"  && <SourceModal src={modal.src} onSave={s => { updateSource(s);     setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "newItem"  && <ItemModal sources={sources} onSave={it => { addItem(it);         setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "editItem" && <ItemModal item={modal.item} sources={sources} onSave={it => { updateItem(it); setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "newMov"   && <MovieModal onSave={m => { addMovie(m);   setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "editMov"  && <MovieModal mov={modal.mov} onSave={m => { updateMovie(m); setModal(null); }} onClose={() => setModal(null)} />}
